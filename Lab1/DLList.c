...
void DLListBefore(DLList L, char *it)
{
	...
	//case of the current item being first item in list (previous node after current being NULL)
	if(L->nitems == 1 || L->curr == L->first){
...
void DLListAfter(DLList L, char *it)
{
	...
	//case of current item being last item in list (next node after current being NULL)
	if(L->nitems == 1 || L->curr == L->last){
		newnode->prev = L->last;
		L->last->next = newnode;
		L->nitems++;
	}else{
...
void DLListDelete(DLList L)
{
	...
	//case no items in list
	if(L->nitems == 0){
		return;
	}
	//case of 1 item in list which is the special case
	if(L->nitems == 1){
		free(L->first->value);
		free(L->first);
		L->first = NULL;
	...
	//case if the current item is the head of the list
	if(L->curr == L->first){
		temp = L->first;
	...
	//case of last item in list being current item
