...
void HashTableStats(HashTable ht)
{
	...
	printf("%8s %8s\n","Length","#Chains");
	int *frequency, max = 0, length; //need a max to workout what is the chain size since it changes
	//dynamically allocating our frequency array which holds our information make it max size of number of items unique
	frequency = malloc(ht->nitems * sizeof(int));	//worst case scanerio is if every single word is packed into 1 linked list, hashtable work well with different enteries
	int i;
	for( i = 0; i<ht->nslots;i++){
		length = ListLength(ht->lists[i]);
		frequency[length]++;				//adds 1 to the frequency for the length of a chain i.e if a list length = 0, add 1 to the #of chains with length 0
		if(length>max) max = length;
	}
	//now from 0 to maximum chain length print out the number of chains
	for( i = 0;i<=max;i++){
